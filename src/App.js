...
function Widget({ market }) {
  ...
  return (
    <div style={{
      ...
      maxWidth: 420,
      boxShadow: "0 4px 24px rgba(0,102,255,0.1)"
    }}>
      {/* Header */}
...
function SearchBar({ onSelect }) {
  ...
  async function search(q) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/.netlify/functions/market?query=${encodeURIComponent(q)}`);
    ...
    <div style={{ position: "relative", width: "100%" }}>
      ...
      {results.length > 0 && (
        <div style={{
          ...
          {results.map((r, i) => (
            <div key={i} onClick={() => { onSelect(r); setQuery(""); setResults([]); }}
              style={{ padding: "12px 16px", cursor: "pointer", borderBottom: "1px solid #0d1f35", color: "#e8edf5", fontSize: 13, transition: "background 0.15s" }}
...
export default function App() {
  ...
  function addMarket(market) {
    setMarkets((prev) => [market, ...prev.filter((m) => m.slug !== market.slug)].slice(0, 6));
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#060a14", color: "#e8edf5", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(180deg, #080d1a 0%, #060a14 100%)", padding: "48px 32px", textAlign: "center", borderBottom: "1px solid #0e1e36" }}>
        <div style={{ fontSize: 42, fontWeight: 800, color: "#fff", letterSpacing: -1, marginBottom: 16, lineHeight: 1.2 }}>
          Put Live Prediction Odds<br />
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 32 }}>
          {[["demo", "Live Demo"], ["how", "How It Works"], ["why", "Why It Matters"]].map(([id, label]) => (
            <button key={id} onClick={() => setTab(id)}
              style={{ background: tab === id ? "#0066ff" : "#0a1628", color: tab === id ? "#fff" : "#4a6a8a", border: `1px solid ${tab === id ? "#0066ff" : "#1a2a4a"}`, borderRadius: 8, padding: "8px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}>
              {label}
            </button>
          ))}
        </div>
        ...
        {tab === "demo" && (
          <div className="fade">
            ...
            <div style={{ marginBottom: 32 }}>
              <div style={{ fontSize: 13, color: "#4a6a8a", marginBottom: 10, letterSpacing: 1, textTransform: "uppercase" }}>
                Search Any Market
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(400px, 1fr))", gap: 24 }}>
              {markets.map((m, i) => (
                <div key={i} className="fade" style={{ animationDelay: `${i * 0.08}s` }}>
                  <Widget market={m} />
                  <button onClick={() => copyEmbed(m.slug)}
                    style={{ marginTop: 8, width: "100%", background: copied === m.slug ? "#003399" : "#0a1628", color: copied === m.slug ? "#fff" : "#4a6a8a", border: "1px solid #1a2a4a", borderRadius: 8, padding: "9px", fontSize: 12, cursor: "pointer", transition: "all 0.2s", fontWeight: 600, letterSpacing: 0.5 }}>
        ...
        {tab === "how" && (
          <div className="fade">
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20 }}>
              {[
                { step: "01", title: "Find a market", desc: "Search for any Polymarket event — elections, sports, crypto, geopolitics. Any topic works." },
                { step: "02", title: "Copy the embed code", desc: "One click copies a single line of HTML. No API keys. No signup. No configuration." },
                { step: "03", title: "Paste on your site", desc: "Drop the code into any article, blog, or webpage. The widget appears instantly with live odds." },
                { step: "04", title: "Odds update live", desc: "The widget fetches real-time data from Polymarket. Your readers always see accurate odds." },
              ].map((s, i) => (
        ...
        {tab === "why" && (
          <div className="fade">
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20 }}>
              {[
                { icon: "📰", title: "For journalists", desc: "Add live prediction odds to your articles. Show readers what the market thinks about the stories you cover." },
                { icon: "⚽", title: "For sports blogs", desc: "Embed live match odds directly in match previews. Your readers see real money predictions, not just pundit opinions." },
                { icon: "📊", title: "For analysts", desc: "Show your audience what prediction markets are pricing in on the topics you analyse. Real data, live." },
                { icon: "🌍", title: "For everyone", desc: "Any website, any topic, any language. If Polymarket has a market for it, you can embed it." },
              ].map((s, i) => (
                <div key={i} style={{ background: "#0a1628", border: "1px solid #1a2a4a", borderRadius: 14, padding: "24px" }}>
        ...
        <div style={{ marginTop: 40, padding: "14px 18px", background: "#0a1628", border: "1px solid #1a2a4a", borderRadius: 8, fontSize: 11, color: "#1e3050", lineHeight: 1.7 }}>
          ⚠ PolyWidget is an independent tool. Not affiliated with Polymarket. Odds are live but may have slight delays. Not financial advice.
        </div>
      </div>
    </div>
  );
